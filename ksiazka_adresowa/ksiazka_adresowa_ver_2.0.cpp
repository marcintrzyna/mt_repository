...
struct Rekord
{
    int idAdresata, id_uzytkownika;
    string imie, nazwisko, telefon, email, adres;

    Rekord(int idAdresata, int id_uzytkownika, string imie, string nazwisko, string telefon, string email, string adres);
};

Rekord::Rekord(int xidAdresata, int xid_uzytkownika, string ximie, string xnazwisko, string xtelefon, string xemail, string xadres)
    : idAdresata (xidAdresata)
    , id_uzytkownika (xid_uzytkownika)
    , imie(ximie)
    , nazwisko(xnazwisko)
    , telefon (xtelefon)
...
void zapisywanie_zmian_do_pliku(vector <Rekord>& dane)
{
    ...
    {
        plik<<dane[i].idAdresata<<"|"<<dane[i].id_uzytkownika<<"|"<<dane[i].imie<<"|"<<dane[i].nazwisko<<"|"<<dane[i].telefon<<"|"<<dane[i].email<<"|"<<dane[i].adres<<"|"<<endl;
    }
    plik.close();
}

void edycja_danych(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    string modyfikacja;
    ...
    else
    {
        ...
        for (int i=0; i<dane.size(); i++)
        {
            if (id_do_modyfikacji == dane[i].idAdresata)
            {
...
int usuwanie_adresata(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    int id_do_usuniecia;
    int ostatnie_id = 0;
    int znaleziono_rekord = 0;
    ...
    else
    {
        ...
        for (int i=0; i<dane.size(); i++)
        {
            if (id_do_usuniecia == dane[i].idAdresata)
            {
                if (idZalogowanegoUzytkownika == dane[i].id_uzytkownika)
                {
                    cout<<endl;
...
void wyswietlanie_wszystkich_danych(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    if (IloscRekordow == 0)
        brak_rekordu();
...
void wyszukaj_Nazwisko(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    string wyszukaj;
    int znaleziono_rekord = 0;
...
void wyszukaj_Imie(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    string wyszukaj;
    int znaleziono_rekord = 0;
...
int dodawanie_adresata(vector <Rekord>& dane, int IloscRekordow, int idZalogowanegoUzytkownika)
{
    string imie, nazwisko, telefon, email, adres;
    int ostatnie_id=0;
    ...
    dane.push_back(Rekord(ostatnie_id+1, idZalogowanegoUzytkownika, imie, nazwisko, telefon, email, adres));
    ...
    fstream plik;
    plik.open("ksiazka_adresowa.txt",ios::out | ios::app);
    plik<<dane[IloscRekordow].idAdresata<<"|"<<idZalogowanegoUzytkownika<<"|"<<dane[IloscRekordow].imie<<"|"<<dane[IloscRekordow].nazwisko<<"|"<<dane[IloscRekordow].telefon<<"|"<<dane[IloscRekordow].email<<"|"<<dane[IloscRekordow].adres<<"|"<<endl;
    plik.close();
...
int wczytywanie_danych_z_pliku(vector <Rekord>& dane, int IloscRekordow)
{
    ...
    for (int i=0; i<bufor.size(); i++)
    {
        ...
        dane.push_back(Rekord(id, id_uzytkownika, imie, nazwisko, telefon, email, adres));
        IloscRekordow++;
    }
...
int rejestracja(vector <Uzytkownik>& uzytkownicy, int IloscUzytkownikow)
{
    ...
    komunikat = "Zarejestrowano uzytkownika o numerze id: " + konwersjaIntNaString(IloscUzytkownikow+1);
    historia_zmian(komunikat);
    Sleep(1000);
...
int main()
{
    ...
    while (true)
    {
        ...
        if (idZalogowanegoUzytkownika != 0)
        {
            ...
            {
                menu_ksiazka_adresowa();
                IloscRekordow = dodawanie_adresata(dane,IloscRekordow,idZalogowanegoUzytkownika);
            }
            else if (wybor == '2')
            {
                menu_ksiazka_adresowa();
                wyszukaj_Imie(dane, IloscRekordow, idZalogowanegoUzytkownika);
            }
            else if (wybor == '3')
            {
                menu_ksiazka_adresowa();
                wyszukaj_Nazwisko(dane, IloscRekordow, idZalogowanegoUzytkownika);
            }
            else if (wybor == '4')
            {
                menu_ksiazka_adresowa();
                wyswietlanie_wszystkich_danych(dane, IloscRekordow, idZalogowanegoUzytkownika);
            }
            else if (wybor == '5')
            {
                menu_ksiazka_adresowa();
                IloscRekordow = usuwanie_adresata(dane, IloscRekordow, idZalogowanegoUzytkownika);
            }
            else if (wybor == '6')
            {
                menu_ksiazka_adresowa();
                edycja_danych(dane, IloscRekordow, idZalogowanegoUzytkownika);
            }
            else if (wybor == '7')
            {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),FOREGROUND_RED | FOREGROUND_INTENSITY);
